...
public class FrequencyQuery {
	...
	public void queryone(String str) throws Exception{
		...
		ResultSet rs=stmt.executeQuery("select frequency from frequencyAuthor where Author='"+str+"'");
		if(rs.next()) {
	...
	public StringBuffer querytopn(int n) throws Exception{
		...
		ResultSet rs=stmt.executeQuery("select top "+n+" * from frequencyAuthor order by frequency DESC");
		int a=0,i,j=-1;
		while(rs.next()) {
			i=j;
			...
			topn.append(a+" "+rs.getString("Author")+"  "+rs.getString("frequency")+"\n");
		}
		stmt.close();
		con.close();
		return topn;
